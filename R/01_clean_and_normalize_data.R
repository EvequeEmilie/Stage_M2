...
#-----------------------------------------------------------------------------
# Identify genes with less than 10 probes
#nProb <- do.call(rbind, lapply(split(geochip, geochip$Gene.name), dim))
#lessThan10 <- rownames(nProb[nProb[, 1] < 10, ])
#maskLessThan10 <- ! geochip$Gene.name %in% lessThan10
# Analyse a better mask
#length_maskLessThan10 <- geochip %>% summarise_all(funs(length))
#length_maskLessThan10
#------------------------------------------------------------------------------
...
finalMask <- (maskLessThan10 + maskCateg) == 2 
